...
export const experienceData: Experience[] = [
    {
        ...
        bullets: [
            ...
            'Splunk 연동 데이터 검증 및 JSON 포맷 변경 대응',
            '40개 이상 문서 분류 체계 기반 라벨링 데이터 추출 및 TF-IDF 변환 지원',
            '개인정보 탐지 쉘 스크립트 개선(파일 조건 제어, 병렬 처리 적용, 예외 디렉토리 관리)'
        ]
    },
    {
        company: 'CSLEE',
        ...
        bullets: [
            'SaaS 보안 데이터 플랫폼을 위한 로그 수집·정제·적재 ETL 파이프라인 설계',
            '이기종 로그(메일·메신저·OCR)를 단일 스키마로 표준화하여 데이터 정합성 확보',
            'Airflow 기반 배치 오케스트레이션 설계 및 의존성 제어 구조 구축',
            '대용량 데이터 처리 중 발생한 메모리·적재 오류에 대해 retry/rollback 체계 설계',
