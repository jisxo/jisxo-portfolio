...
export const restaurantProject: Project = {
    ...
    domain: 'Commercial Real Estate & Analytics',
    tags: ['Python', 'Docker', 'Apache Airflow', 'PostgreSQL', 'Web Crawling (Selenium/BS4)', 'Git'],
    star: {
        situation: `서울시 상권 분석 및 입지 컨설팅을 위한 핵심 데이터(매출, 유동인구, 평점 등)를 확보하여, 이를 **판매 가능한 데이터 상품**으로 자산화하는 것이 목표였습니다.
하지만 유의미한 데이터가 공공 API와 포털 사이트로 파편화되어 있고, 데이터 간 결합 기준이 없어 **신뢰도 높은 상권 분석 지표** 산출이 불가능한 상황이었습니다.`,
        action: `데이터 파이프라인 구축을 위해 서울시 공공데이터와 포털 크롤러를 개발하여 일일/주간 단위로 데이터를 수집·적재하는 **자동화 시스템**을 구현했습니다.
수집된 Raw Data를 분석 목적에 맞게 정규화하여 **데이터 마트로 재설계**하고, 상세한 'DB 인터페이스 정의서'를 작성하여 데이터 활용도를 극대화했습니다.`,
        result: `흩어져 있던 서울시 전역의 데이터를 표준화된 DB로 통합하여, **데이터 자산화** 및 **입지 분석 리포트** 등 신규 비즈니스 모델의 기술적 토대를 마련했습니다.
최적화된 데이터 마트 구축을 통해 수천만 건의 데이터 연산 부하를 최소화하고, **실시간 랭킹 산출** 및 대시보드 조회가 가능한 분석 환경을 완성했습니다.`
    },
    contributions: [
