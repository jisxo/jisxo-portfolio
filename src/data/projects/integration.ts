...
export const integrationProject: Project = {
    ...
    star: {
        ...
        action: `로그별 Key 값을 기준으로 중복을 제거하는 전처리 로직을 개발하여 데이터 정합성을 확보하고, 통합 스키마 설계 및 정규표현식 활용 파싱 모듈을 통해 비정형 로그를 표준 포맷으로 자동 변환하는 ETL 파이프라인을 구축했습니다.`,
        result: `이를 통해 원천 데이터와 적재 데이터 간의 정합성 100%를 달성했으며, 오탐을 최소화하여 이상탐지 모니터링 업무 효율을 높이고 향후 신규 장비 추가 시 즉시 대응 가능한 확장성 있는 데이터 구조를 완성했습니다.`
    },
    contributions: [
