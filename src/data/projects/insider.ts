...
export const insiderProject: Project = {
    ...
    star: {
        situation: `사내 메신저와 이메일 등 방대한 비정형 데이터를 분석해 내부자 위협(Insider Threat)을 사전에 탐지해야 했으나, 기존의 수동 분석 방식으로는 실시간 대응이 불가능했으며 대용량 텍스트 처리 시 심각한 성능 병목이 발생했습니다.`,
        action: `Apache Airflow를 도입하여 전 과정을 자동화 파이프라인으로 구축했습니다. 대용량 텍스트 벡터화 시 발생하는 메모리 초과 문제를 해결하기 위해 JSON 파일 분할 처리 기법을 적용하여 안정성을 확보했습니다.
또한 단순 키워드 매칭을 넘어 문맥(Context) 기반의 이상징후를 포착할 수 있도록 고품질의 텍스트 학습 데이터셋을 체계적으로 구축했습니다.`,
        result: `비정형 데이터 분석 워크플로우를 100% 자동화하여 운영 리소스를 최소화했습니다.
지연 없는 실시간 처리 체계를 완성함으로써, 기술 유출 시도를 선제적으로 방어하고 기업 평판 및 핵심 자산을 보호하는 강력한 보안 관제 체계를 확립했습니다.`
    },
    contributions: [
