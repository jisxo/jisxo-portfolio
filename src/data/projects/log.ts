...
export const logProject: Project = {
    ...
    overview: {
        background: "초기 보안 관제는 수천 건의 외부 메일 로그를 담당자가 직접 룰베이스로 사후 분석하는 비효율적인 구조였으며, 정량적인 탐지 기준이 부족해 과도한 수동 업무가 수반되었습니다.",
        objective: "로그 수집 및 전처리를 100% 자동화하는 신규 분석 체계를 구축하고, 통계적 EDA와 AI 모델을 결합해 매일 확인해야 하는 이상징후 건수를 실질적으로 줄이는 데 집중했습니다.",
