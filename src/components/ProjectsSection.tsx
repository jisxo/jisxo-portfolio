...
const projects = [
    {
        ...
        description: '외부 발송 메일 로그의 수집·전처리 과정을 100% 자동화하고, 탐색적 데이터 분석을 통해 내부 탐지 모델의 기초를 마련한 데이터 엔지니어링 프로젝트.',
        image: 'https://placehold.co/600x400/101113/007bff?text=Log+Analysis',
        tags: ['Python', 'Docker', 'Apache Airflow', 'Pandas', 'ETL Pipeline', 'Data Analysis (EDA)', 'Anomaly Detection'],
        star: {
            situation: '매일 발생하는 대량의 외부 발송 메일 로그를 분석하여 이상 행위 징후를 포착해야 했으나, 정제된 데이터 수집 체계가 없어 수동 분석에 의존하는 비효율이 존재했습니다.',
            action: 'Python 기반의 전처리 모듈을 직접 개발하여 기존 배치 솔루션과 연동시킴으로써 데이터 수집부터 가공까지의 전 과정을 100% 자동화하고, 시간/부서/키워드별 다차원 탐색적 데이터 분석을 수행하여 이상징후 탐지 룰 설정을 위한 통계적 근거를 확보했습니다.',
            result: '이를 통해 수동 데이터 추출에 소요되던 공수를 제로화하여 분석 효율을 극대화했으며, 통계적 근거 기반의 정량적 탐지 룰을 도출하여 보안 관제 고도화의 핵심 기반을 완성했습니다.'
        }
    },
    {
        ...
        tags: ['Python', 'Docker', 'Apache Airflow', 'ETL Pipeline', 'Log Analysis Platform', 'Data Standardization'],
        star: {
            situation: "출력물 보안 솔루션, 외부 메일 등 5종 이상의 이기종 보안 로그를 통합 분석해야 했으나, 장비마다 상이한 로그 포맷과 데이터 전송 과정에서 발생하는 '데이터 중복 적재' 이슈로 인해 분석 신뢰도가 심각하게 훼손된 상황이었습니다.",
            action: '로그별 고유 키를 기준으로 중복을 제거하는 Python 기반 전처리 로직을 개발하여 데이터 정합성을 확보하고, 통합 스키마 설계 및 정규표현식 파싱 모듈을 통해 비정형 로그를 표준 포맷으로 자동 변환하는 파이프라인을 구축했습니다. 또한, 부서별 업무 특성을 분석하여 탐지 임계치를 최적화했습니다.',
    ...
    },
    {
        ...
        star: {
            situation: '내부자 위협이 높아짐에 따라, 사내 협업 툴과 이메일 등 방대한 비정형 데이터를 분석해 이상 행위를 사전에 차단해야 했습니다. 하지만 기존 시스템은 자동화가 되어 있지 않았고, 대용량 텍스트 벡터화 처리 시 성능 병목이 발생했습니다.',
            action: "Apache Airflow를 도입하여 전 과정을 자동화 파이프라인으로 구축했습니다. 이 과정에서 AI 모델 학습을 위한 대용량 텍스트 벡터화 수행 시 메모리 초과 및 속도 저하가 발생하자, 거대한 JSON 데이터를 작은 단위로 쪼개어 처리하는 파일 분할 기법을 적용해 병목 현상을 기술적으로 해결했습니다. 또한 사용자 행위 기반의 이상징후 패턴 탐지를 위해 단순 키워드가 아닌 '문맥'을 포함한 문장 학습데이터셋을 구축하여 탐지 정교성을 높였습니다.",
    ...
    },
    {
        ...
        star: {
            situation: '통신사 특성상 하루에도 수백만 건의 보안 로그가 발생하는데, 특히 문서 검사 로직이 비효율적이어서 데이터 처리에 2시간 이상 소요되는 지연 이슈가 발생했습니다. 또한, AI 탐지 모델 고도화를 위한 양질의 학습 데이터가 부족한 상황이었습니다.',
            action: '처리 속도 개선을 위해 기존의 일괄 처리 방식을 1,000개 단위의 청크 기반 분할 처리 로직으로 변경하여 메모리 부하를 줄이고 속도를 최적화했습니다. 동시에 비정형 문서 데이터를 AI가 학습할 수 있도록 단어 빈도-역문서 빈도 알고리즘을 적용해 벡터화하고, 핵심 키워드를 추출하여 정교한 라벨링 데이터셋을 구축했습니다.',
            result: '이를 통해 로그 처리 시간을 획기적으로 단축하여 데이터 병목 현상을 해결했으며, 대시보드 개선 및 양질의 학습 데이터셋 공급 체계를 완성하여 보안 관제 효율성을 높이고 AI 분석을 위한 핵심 기반을 마련했습니다.'
        }
    },
    {
        ...
        images: [
            { src: '/images/rpa_architecture.png', caption: 'RPA 시스템 아키텍처' },
            { src: '/images/rpa_batch_schedule.png', caption: 'Airflow 배치 스케줄링 및 모니터링' },
            { src: '/images/rpa_fe.png', caption: 'RPA 운영/관리 대시보드' }
        ],
        tags: ['Python', 'Docker', 'Apache Airflow', 'Django', 'Amazon API'],
        star: {
            situation: '스마트폰 액세서리 브랜드의 아마존 셀러 광고 업무를 담당하며, 미국/일본 등 글로벌 10개국 마켓의 광고 캠페인을 1명이 관리해야 했습니다. 단순 규칙 기반의 반복 업무가 월 150시간 이상 소요되었으며, 노동집약적 업무로 인한 휴먼 에러 위험이 높았습니다.',
    ...
    },
    {
        ...
        images: [
            ...
            { src: '/images/hipp_db_desc.png', caption: 'DB 인터페이스 명세서' },
